...
CONFIG_FILE = "env-tester-tapad-graph.yaml"
...
def get_kubernetes_pod_operator(step_config, dag, config):
    return dict(
      ...
      operator=KubernetesPodOperator(
        ...
        node_selectors=list(map(get_env, get_key("node_selectors", step_config))),
        image=step_config["image"],
        cmds=list(get_key("cmds", step_config)),
        env_vars=list(map(get_env, get_key("env_vars", step_config))),
